...
def create_venn_diagrams(decoy_transdecoder, decoy_genemark, transdecoder, genemark, prefix):
    ...
    total_v3 = len(set_td.union(set_td_decoy))
    v3 = venn2([set_td, set_td_decoy], set_labels=('Transdecoder', 'Transdecoder decoy'), ax=axes[1][0],
               subset_label_formatter=lambda x: f"{(x/total_v3):.2%}")
    total_v4 = len(set_gm.union(set_gm_decoy))
    ...
    plt.tight_layout()
    plt.savefig('output/comparison_graphs/sample_{}_percentage.png'.format(prefix))
...
def main(argv):
    ...
    try:
        ...
        print("usage: venn_percentage_test.py -g <genemark csv file> -t <transdecoder csv file> "
              "-h <decoy genemark file> -u <decoy transdecoder file> -p <output prefix>")
        sys.exit(2)
